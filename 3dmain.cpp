...
#endif
  g_drawbuff[0] = (uint16_t*)malloc(sizeof(uint16_t)*window_width*DRAW_NLINES);
  g_drawbuff[1] = (uint16_t*)malloc(sizeof(uint16_t)*window_width*DRAW_NLINES);
  g_zbuff = (uint16_t*)malloc(sizeof(uint16_t)*window_width*DRAW_NLINES);

  // t = new texturetriangle[MAXPROC_POLYNUM];
  for(int i=0;i<MAXPROC_POLYNUM/TRI_MULTI;i++){
    tri[i] = (texturetriangle*)malloc(sizeof(texturetriangle)*TRI_MULTI);
